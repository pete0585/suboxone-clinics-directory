...
export async function POST(req: NextRequest) {
  ...
  // Resend delivers email.received events via Svix envelope: { type: "email.received", data: { from, to, subject, text, html } }
  ...
  if (!listingId && bodyText) {
    const slugMatch = bodyText.match(/suboxoneclinicfinder\.com\/clinic\/([\w-]+)/)
