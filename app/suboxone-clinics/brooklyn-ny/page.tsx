...
export default async function CityPage() {
  ...
  const faqLd = {
    ...
    mainEntity: [
      {
        "@type": "Question",
        name: "How many suboxone clinic are in Brooklyn, NY?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Suboxone Clinic Directory lists 18+ suboxone clinic in Brooklyn, New York. Counts change as new listings are seeded.",
        },
      },
      {
        "@type": "Question",
        name: "How do I find suboxone clinic in Brooklyn?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Search suboxoneclinicfinder.com and filter by Brooklyn. Compare listed providers, then contact the one that fits.",
        },
      },
    ],
  }
  return (
    <>
      ...
      <main className="mx-auto max-w-5xl px-4 py-10">
        <p className="text-sm text-neutral-500">Brooklyn, NY</p>
        <h1 className="mt-2 text-3xl font-bold tracking-tight">
          Suboxone Clinic in Brooklyn, NY
        </h1>
        <p className="mt-3 max-w-2xl text-neutral-600">
          18+ listed suboxone clinic in the Brooklyn area. Pages are generated from live directory listings — not outreach.
        </p>
          ...
          {listings.map((row: Record<string, unknown>, i: number) => (
            <li key={String(row.id || row.slug || i)} className="rounded-xl border border-neutral-200 p-4">
              ...
              <p className="mt-1 text-sm text-neutral-500">
                {String(row.city || "Brooklyn")}, {String(row.state || "NY")}
              </p>
            </li>
          ))}
        </ul>
